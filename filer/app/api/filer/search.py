from fastapi import APIRouter, UploadFile, File, Header, Depends, HTTPException, Query
import os
import shutil
import boto3
import jwt
...
async def search_files(
    ...
    query: str = Query(..., title="Search Query", description="Enter the search query")
):
    try:
        files = list_files()

        # Filter files based on search_query
        filtered_files = [file for file in files if query.lower() in file.lower()]

        return {"search_query": query, "filtered_files": filtered_files}
